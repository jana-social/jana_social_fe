...
RSpec.describe "Event Update Page" do
  before(:each) do
  ...
  describe "display" do
    it "has a link to update an event" do
      ...
      click_on("Casey's Sweet Quiet Gathering")
      expect(page).to have_link("Update Event")
      click_on("Update Event")

      expect(current_path).to eq(event_edit_path(1, 1))
    end
  end
end